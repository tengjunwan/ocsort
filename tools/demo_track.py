...
class YOLOv8Predictor:
    ...
    def __init__(self, model_path="pretrained/yolov8n_visdrone_2.pt", device="cuda"):
        ...
        self.device = device
        self.model.fuse()
        self.vehicle_ids = [0, 1, 2]  # car, bus, truck (COCO IDs)
        self.imgsz = 960
    ...
    def inference(self, img, timer=None):
        ...
        # Run detection
        if timer: timer.tic()
        results = list(self.model(img, imgsz=self.imgsz, verbose=False))
        ...
        return [torch.tensor(output, device=self.device)], img_info
...
def image_demo(predictor, vis_folder, current_time, args):
    ...
    logger.info(f"score thresh:{args.track_thresh}, iou thresh:{args.iou_thresh}")
    timer = Timer()
    results = []
