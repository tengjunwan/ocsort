...
class YoloPredictor():
    ...
    def __init__(self, onnx_path="pretrained/yolov8n_visdrone_2.onnx",
                 conf_threshold = 0.25,
                 nms_threshold = 0.7
                 ):
...
# # Load ONNX model
# session = onnxruntime.InferenceSession("pretrained/yolov8n_visdrone_2.onnx", providers=["CPUExecutionProvider"])
...
# # Preprocess image
# # img = cv2.imread("imgs/uav0000306_00230_v/0000001.jpg")
# img = cv2.imread("frame_0_640_640.png")
# img_resized = cv2.resize(img, (640, 640))
# img_input = cv2.cvtColor(img_resized, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
# img_input = img_input.transpose(2, 0, 1)[np.newaxis]  # [1, 3, 640, 640]

# # Run ONNX inference
# output = session.run([output_name], {input_name: img_input})[0]  # [1, 7, 8400]
# output = np.squeeze(output)  # [7, 8400]

# # Postprocess
# boxes = output[:4, :].T  # [8400, 4]
# cls_scores = output[4:, :].T  # [8400, 3]
# confidences = np.max(cls_scores, axis=1)  # [8400]
...
#     cv2.putText(img_resized, label, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)

# cv2.imwrite("result_yuv.jpg", img_resized)
# print(f"result saved: 'result_yuv.jpg'")
